...
class valid_point(beam.DoFn):
    def process(self, element):
        ...
        #logging.info("*** Point Data =  %s", element)
        #logging.info("*** Point Data =  %s", df.values)

        # check for source
        if (df['key'].str.contains("tweets").any()): logging.info("*** Tweet ***")
        if (df['key'].str.contains("traffic").any()): logging.info("*** Traffic ***")
        if (df['key'].str.contains("uv").any()): logging.info("*** Weather ***")
        ...
        if (df['key'].str.contains("tweets").any() and df['key'].str.contains("uv").any() and
            ...
            tweets_i = np.where(df['key'].values == 'tweets')
            precip_i = np.where(df['key'].values == 'precip')
            temp_i = np.where(df['key'].values == 'temp')
            uv_i = np.where(df['key'].values == 'uv')
            wind_i = np.where(df['key'].values == 'wind')
            traffic_i = np.where(df['key'].values == 'traffic')

            tweets_value = df['value'].iloc[tweets_i].mean(axis=0)
            precip_value = df['value'].iloc[precip_i].mean(axis=0)
            temp_value = df['value'].iloc[temp_i].mean(axis=0)
            uv_value = df['value'].iloc[uv_i].mean(axis=0)
            wind_value = df['value'].iloc[wind_i].mean(axis=0)
            traffic_value = df['value'].iloc[traffic_i].mean(axis=0)

            # clustering order: tweets, precip, temp, uv, wind, traffic
            yield [tweets_value, precip_value, temp_value, uv_value, wind_value, traffic_value]
            # yield [(float(10), float(0.1), float(57.9), float(0.0), float(8.6), float(0.25))]
            # yield element

        else:
            #logging.info("*** Partial Point ***")
            return
...
class normalize_point(beam.DoFn):
    ...
    def process(self, element):
        ...
        # max/min are based on 10 minute sample. Using window duration in Beam to normalize value.
        window_duration = 0.5 # duration of windowed tweet count in minutes


        logging.info("++Observation++ %s", element)

        tweet = ((element[0] * 10 / window_duration)- min_tweets) / (max_tweets - min_tweets)
        precip = element[1]
        temp = (element[2] - min_temp) / (max_temp - min_temp)
        uv = (element[3] - min_uv) / (max_uv - min_uv)
        wind = (element[4] - min_wind) / (max_wind - min_wind)
        traffic = (element[5] - image_min) / (image_max - image_min)
        # clustering order: tweets, precip, temp, uv, wind, traffic
...
traffic_stream = ( p | "Read Image" >> beam.io.ReadFromPubSub(subscription=image_sub, with_attributes=True)
                    | "Traffic Window" >> beam.WindowInto(window.SlidingWindows(30,10))
                    #                    trigger=beam.trigger.Repeatedly(beam.trigger.AfterAny(
                    #                            beam.trigger.AfterCount(1))),
                    #                            #beam.trigger.AfterProcessingTime(10))),
...
twitter_stream = ( p | "Read Twitter" >> beam.io.ReadFromPubSub(subscription=tweet_sub, with_attributes=True)
                    | "Twitter Window" >> beam.WindowInto(window.SlidingWindows(30,10))
                    #                    trigger=beam.trigger.Repeatedly(beam.trigger.AfterAny(
                    #                            #beam.trigger.AfterCount(1))),
                    #                            beam.trigger.AfterProcessingTime(10))),
                    #                    accumulation_mode=beam.trigger.AccumulationMode.ACCUMULATING)
...
weather_stream = ( p | "Read Weather" >> beam.io.ReadFromPubSub(subscription=weather_sub, with_attributes=True)
                    | "Weather Window" >> beam.WindowInto(window.SlidingWindows(30,10))
                    #                    trigger=beam.trigger.Repeatedly(beam.trigger.AfterAny(
                    #                            beam.trigger.AfterCount(1))),
                    #                            #beam.trigger.AfterProcessingTime(10))),
                    #                    accumulation_mode=beam.trigger.AccumulationMode.ACCUMULATING)
...
merged_stream = ((traffic_stream, weather_stream, twitter_stream) | "Join" >> beam.Flatten())

candidate_points = ( merged_stream
                    #| "Window" >> beam.WindowInto(window.FixedWindows(60))#,
                    #                  trigger=beam.trigger.Repeatedly(
                    #                      beam.trigger.AfterAny(
                    #                          beam.trigger.AfterCount(10))),
                    #                  accumulation_mode=beam.trigger.AccumulationMode.ACCUMULATING)
                    | 'Add Key' >> beam.ParDo(lambda (c): [("key", c)])
                    | 'Group' >> beam.GroupByKey())
                    #| 'Log Potential Point' >> beam.ParDo(lambda (c): logging.info("*** Output=  %s",c)))

valid_points = ( candidate_points
                    | "Valid Point" >> beam.ParDo(valid_point())
                    | "Normalize Point" >> beam.ParDo(normalize_point())
                    | "Assign Cluster" >> beam.ParDo(assign_cluster())
                    | 'Log Cluster' >> beam.ParDo(lambda (a,c): logging.info("*** Observed Cluster =  %s",c)))
                    #| 'Log Valid Point' >> beam.ParDo(lambda (c): logging.info("*** Data =  %s",c)))
