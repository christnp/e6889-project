...
#
    ...
    #except:
        ...
        logging.error(('Topic not found: create_subscription() threw '
                'exception: {}').format(e))
        raise
        # RB: I hit this exception immediately the first time I ran the code.
        #     Nick, you probably create the topic before hand on the Pub/Sub
        #     dashboard, but we need to add a create_topic(). Doing so here
        #     requires more code to try the create_subscription() again, hence
        #     the create_topic() above.
        #
        #publisher = pubsub_v1.PublisherClient()
        #try:
        #    logging.info('Creating new topic {}'.format(grtopic_path))
        #    publisher.create_topic(grtopic_path)
        #except AlreadyExists:
        #    pass
        #except Exception as e:
